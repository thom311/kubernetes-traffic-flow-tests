...
class TestConfig:
    ...
    def __init__(
        ...
        *,
        ...
        kubeconfig_pair1: tuple[Optional[str], Optional[str]]
        if kubeconfigs is not None:
            kubeconfig_pair1 = kubeconfigs
            kubeconfigs_cwd = cwddir
        else:
            kubeconfig_pair1 = (self.config.kubeconfig, self.config.kubeconfig_infra)
            kubeconfigs_cwd = configdir
        kubeconfig1, kubeconfig_infra1 = kubeconfig_pair1
        assert kubeconfig1 is None or kubeconfig1
        assert kubeconfig_infra1 is None or kubeconfig_infra1
        assert kubeconfig_infra1 is None or kubeconfig1 is not None
        if kubeconfig1 is not None:
            self._kubeconfig_pair = (
                common.path_norm(kubeconfig1, cwd=kubeconfigs_cwd),
                common.path_norm(kubeconfig_infra1, cwd=kubeconfigs_cwd),
            )
        else:
